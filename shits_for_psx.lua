getgenv().LuauClass = (function()
    ...
    function j.new(k, ...)
        ...
        if k.super then
            k.super._should_init = false
            self.super = k.super.new()
        end
        if k._should_init then
            self:__init(...)
        end
        return self
    end
...
getgenv().Promise =
    ...
    function()
        ...
        do
            ...
            function Error:__tostring()
                ...
                for _, runtimeError in ipairs(self:getErrorChain()) do
                    table.insert(
                        errorStrings,
                        table.concat({runtimeError.trace or runtimeError.error, runtimeError.context}, "\n")
                    )
                end
        ...
        function Promise.is(object)
            ...
            elseif
                type(objectMetatable) == "table" and type(rawget(objectMetatable, "__index")) == "table" and
                    isCallable(rawget(rawget(objectMetatable, "__index"), "andThen"))
             then
                return true
            end
            return false
